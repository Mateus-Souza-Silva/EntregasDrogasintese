...
public class CadastrarSetor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        setor.setDescricao(request.getParameter("descricao").toUpperCase());
        
        try {
            GenericDAO dao = new SetorDAOImpl();
            if (dao.cadastrar(setor)) {
                mensagem = "Setor cadastrado com sucesso!";
            }else{
                mensagem = "Problemas ao cadastrar Setor CONTROLLER!";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("Setor/cadastrar.jsp").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Problemas ao cadastrar Setor CONTROLLER! Erro: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
